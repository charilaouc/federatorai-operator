...
// Add creates a new AlamedaService Controller and adds it to the Manager.
// The Manager will set fields on the Controller and
...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	c, err := controller.New(
		"alamedaservice-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}
	// Watch for changes to primary resource AlamedaService
	err = c.Watch(&source.Kind{
		Type: &federatoraiv1alpha1.AlamedaService{},
...
func (r *ReconcileAlamedaService) Reconcile(
	request reconcile.Request) (reconcile.Result, error) {

	instance := &federatoraiv1alpha1.AlamedaService{}
	err := r.client.Get(context.TODO(), request.NamespacedName, instance)
	if err != nil && !k8sErrors.IsNotFound(err) {
		log.V(-1).Info("Get AlamedaService failed, retry reconciling.",
		...
		log.Info("Handing AlamedaService deletion.", "AlamedaService.Namespace",
			request.Namespace, "AlamedaService.Name", request.Name)
		if err := r.handleAlamedaServiceDeletion(request); err != nil {
			log.V(-1).Info("Handle AlamedaService deletion failed, retry reconciling.",
				"AlamedaService.Namespace", request.Namespace, "AlamedaService.Name",
				request.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: requeueAfter}, nil
		}
		log.Info("Handle AlamedaService deletion done.", "AlamedaService.Namespace",
			request.Namespace, "AlamedaService.Name", request.Name)
		return reconcile.Result{}, nil
	}

	log.Info("Reconciling AlamedaService.", "AlamedaService.Namespace",
	...
	if err != nil {
		log.V(-1).Info("check if AlamedaService needs to reconcile failed, retry reconciling",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if !isNeedToBeReconciled {
		log.Info("AlamedaService does not need to be reconcile", "AlamedaService.Namespace",
			instance.Namespace, "AlamedaService.Name", instance.Name)
		err := r.updateAlamedaServiceActivation(instance, false)
		if err != nil {
			log.V(-1).Info("Update AlamedaService activation failed, retry reconciling",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
		}
	...
	if !hasGCOwner {
		...
		if err = r.client.Get(
			context.TODO(), request.NamespacedName, tmpInstance); err != nil {
	...
	hasSpecBeenChanged, _ := r.checkAlamedaServiceSpecIsChange(
		instance, request.NamespacedName)
	if !hasSpecBeenChanged &&
		util.Disable_operand_resource_protection == "true" && !isFirstReconciled {
		log.Info("AlamedaService spec is not changed, skip reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace,
	...
	if err != nil {
		log.V(-1).Info("Get Namespace failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	...
	if err != nil {
		log.V(-1).Info("New ComponentConfig failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	...
	if asp.AutoPatchPrometheusRules {
		if ok, missingRulesMap, err := prom_patch.RulesCheck(
			...
			log.V(-1).Info("check prometheusrules failed, retry reconciling AlamedaService",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			...
			if err = prom_patch.PatchMissingRules(
				...
				log.V(-1).Info("patch prometheusrules failed, retry reconciling AlamedaService",
					"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
					instance.Name, "msg", err.Error())
				return reconcile.Result{Requeue: true, RequeueAfter: 10 * time.Second}, nil
			}
		...
		/*
			if ok, err := prom_patch.RelabelingCheck(prom_patch.GetK8SClient()); err != nil {
				log.V(-1).Info("check relabelings failed, retry reconciling AlamedaService",
					"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
					instance.Name, "msg", err.Error())
				return reconcile.Result{Requeue: true, RequeueAfter: 10 * time.Second}, nil
			} else if !ok {
				if err = prom_patch.PatchRelabelings(prom_patch.GetK8SClient()); err != nil {
					log.V(-1).Info("patch relabeling failed, retry reconciling AlamedaService",
						"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
						instance.Name, "msg", err.Error())
					return reconcile.Result{Requeue: true, RequeueAfter: 10 * time.Second}, nil
				}
	...
	if err := r.syncPodSecurityPolicy(
		instance, clusterRoleGC, asp, installResource); err != nil {
		log.V(-1).Info("Sync podSecurityPolicy failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.syncSecurityContextConstraints(
		...
		log.V(-1).Info("Sync securityContextConstraint failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: requeueAfter}, nil
	}
	...
	if err := r.createServiceAccount(instance, asp, installResource); err != nil {
		log.V(-1).Info("Create serviceAccount failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	...
	if err := r.syncRole(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync Role failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.syncRoleBinding(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync RoleBinding failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.createSecret(instance, asp, installResource); err != nil {
		log.V(-1).Info("create secret failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.createPersistentVolumeClaim(instance, asp, installResource); err != nil {
		log.V(-1).Info("create PersistentVolumeClaim failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.syncConfigMap(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync configMap failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.syncService(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync service failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.syncServiceExposure(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync service exposure failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	...
	if err := r.syncDeployment(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync deployment failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	...
	if err := r.syncStatefulSet(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync statefulset failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.syncIngress(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync Ingress failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if err := r.syncRoute(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync route failed, retry reconciling.", "AlamedaService.Namespace",
	...
	if err := r.syncDaemonSet(instance, asp, installResource); err != nil {
		log.V(-1).Info("Sync DaemonSet failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	...
	if !asp.EnableExecution {
		...
		if err := r.uninstallResource(*excutionResource); err != nil {
			log.V(-1).Info("Uninstall execution resources failed, retry reconciling AlamedaService",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
		}
	...
	if !asp.EnableDispatcher {
		...
		if err := r.uninstallResource(resource); err != nil {
			log.V(-1).Info("Uninstall dispatcher resources failed, retry reconciling AlamedaService",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
		}
	...
	if !asp.EnablePreloader {
		...
		if err := r.uninstallResource(resource); err != nil {
			log.V(-1).Info("Uninstall preloader resources failed, retry reconciling AlamedaService",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
		}
	...
	if !asp.EnableWeavescope {
		...
		if err := r.uninstallResource(resource); err != nil {
			log.V(-1).Info("Uninstall weavescope resources failed, retry reconciling AlamedaService",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
		}
	...
	if !asp.EnableVPA {
		...
		if err := r.uninstallResource(resource); err != nil {
			log.V(-1).Info("Uninstall vpa resources failed, retry reconciling AlamedaService",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
		}
	...
	if err := r.uninstallPersistentVolumeClaim(instance, pvcResource); err != nil {
		log.V(-1).Info("retry reconciling AlamedaService", "AlamedaService.Namespace",
			instance.Namespace, "AlamedaService.Name", instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}
	if !asp.SelfDriving {
		...
		if err := r.createScalerforAlameda(instance, asp, installResource); err != nil {
			log.V(-1).Info("create scaler for alameda failed, retry reconciling AlamedaService",
				"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
				instance.Name, "msg", err.Error())
			return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
		}
	...
	if err = r.updateAlamedaService(instance, request.NamespacedName, asp); err != nil {
		log.Error(err, "Update AlamedaService failed, retry reconciling AlamedaService",
			"AlamedaService.Namespace", instance.Namespace, "AlamedaService.Name",
			instance.Name, "msg", err.Error())
		return reconcile.Result{Requeue: true, RequeueAfter: 1 * time.Second}, nil
	}

	log.Info("Reconciling done.", "AlamedaService.Namespace", instance.Namespace,
		"AlamedaService.Name", instance.Name)
...
func (r *ReconcileAlamedaService) handleAlamedaServiceDeletion(
...
func (r *ReconcileAlamedaService) InitAlamedaService(
...
func (r *ReconcileAlamedaService) getNamespace(namespaceName string) (
...
func (r *ReconcileAlamedaService) newComponentConfig(
	namespace corev1.Namespace,
	alamedaService federatoraiv1alpha1.AlamedaService,
...
func (r *ReconcileAlamedaService) createScalerforAlameda(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	...
	for _, fileString := range resource.AlamedaScalerList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource Scaler... ",
			...
			log.Info("Successfully Creating Resource Scaler",
				"resourceScaler.Name", resourceScaler.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) syncCustomResourceDefinition(
	instance *federatoraiv1alpha1.AlamedaService,
...
func (r *ReconcileAlamedaService) uninstallCustomResourceDefinition(
...
func (r *ReconcileAlamedaService) syncClusterRoleBinding(
	instance *federatoraiv1alpha1.AlamedaService,
	...
	for _, FileStr := range resource.ClusterRoleBindingList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource ClusterRoleBinding... ",
			...
			log.Info("Successfully Creating Resource ClusterRoleBinding",
				"resourceCRB.Name", resourceCRB.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) createAlamedaNotificationChannels(
...
func (r *ReconcileAlamedaService) createAlamedaNotificationTopics(
	owner metav1.Object, resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) syncPodSecurityPolicy(
	instance *federatoraiv1alpha1.AlamedaService,
	gcIns *rbacv1.ClusterRole,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	...
	for _, FileStr := range resource.PodSecurityPolicyList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource PodSecurityPolicy... ",
			...
			log.Info("Successfully Creating Resource PodSecurityPolicy",
				"resourcePSP.Name", resourceMeta.GetName())
		} else if err != nil {
...
func (r *ReconcileAlamedaService) syncSecurityContextConstraints(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.SecurityContextConstraintsList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource SecurityContextConstraints... ",
			...
			log.Info("Successfully Creating Resource SecurityContextConstraints",
				"resourceSCC.Name", resourceSCC.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) syncDaemonSet(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.DaemonSetList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource DaemonSet... ",
			...
			log.Info("Successfully Creating Resource DaemonSet",
				"resourceDS.Namespace", resourceDS.Namespace,
				"resourceDS.Name", resourceDS.Name)
		} else if err != nil {
			...
			if updateresource.MisMatchResourceDaemonSet(foundDS, resourceDS) {
				log.Info("Update Resource DaemonSet:",
				...
				log.Info("Successfully Update Resource DaemonSet",
					"resourceDS.Namespace", resourceDS.Namespace,
					"resourceDS.Name", resourceDS.Name)
			}
		}
...
func (r *ReconcileAlamedaService) syncClusterRole(
	instance *federatoraiv1alpha1.AlamedaService,
	gcIns *rbacv1.ClusterRole, asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.ClusterRoleList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource ClusterRole... ",
			...
			log.Info("Successfully Creating Resource ClusterRole",
				"resourceCR.Name", resourceCR.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) createServiceAccount(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.ServiceAccountList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource ServiceAccount... ",
			...
			log.Info("Successfully Creating Resource ServiceAccount",
				"resourceSA.Name", resourceSA.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) syncRole(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.RoleList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource Role... ", "resourceCR.Namespace",
			...
			log.Info("Successfully Creating Resource Role",
...
func (r *ReconcileAlamedaService) syncRoleBinding(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.RoleBindingList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource RoleBinding... ",
				"resourceCR.Namespace", resourceCR.Namespace,
				"resourceCR.Name", resourceCR.Name)
			err = r.client.Create(context.TODO(), resourceCR)
			if err != nil {
			...
			log.Info("Successfully Creating Resource RoleBinding",
				"resourceCR.Namespace", resourceCR.Namespace,
				"resourceCR.Name", resourceCR.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) createPersistentVolumeClaim(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.PersistentVolumeClaimList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource PersistentVolumeClaim... ",
			...
			log.Info("Successfully Creating Resource PersistentVolumeClaim",
				"resourcePVC.Name", resourcePVC.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) createSecret(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) syncConfigMap(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.ConfigMapList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource ConfigMap... ",
			...
			log.Info("Successfully Creating Resource ConfigMap",
				"resourceCM.Name", resourceCM.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) syncService(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.ServiceList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource Service... ",
			...
			log.Info("Successfully Creating Resource Service", "resourceSV.Name",
				resourceSV.Name)
		} else if err != nil {
			...
			if updateresource.MisMatchResourceService(foundSV, resourceSV) {
				...
				log.Info("Successfully Update Resource Service",
...
func (r *ReconcileAlamedaService) syncServiceExposure(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) newServiceByServiceExposure(svc corev1.Service,
...
func (r *ReconcileAlamedaService) newNodePortServiceByServiceExposure(
	svc corev1.Service,
...
func (r *ReconcileAlamedaService) syncMutatingWebhookConfiguration(
	instance *federatoraiv1alpha1.AlamedaService,
	gcIns *rbacv1.ClusterRole, asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.MutatingWebhookConfigurationList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating MutatingWebhookConfiguration...", "name",
			...
			log.Info("Updating MutatingWebhookConfiguration", "name",
				mutatingWebhookConfiguration.Name)
			...
			log.Info("Updating MutatingWebhookConfiguration done.", "name",
				mutatingWebhookConfiguration.Name)
		}
	}
	return nil
...
func (r *ReconcileAlamedaService) syncValidatingWebhookConfiguration(
	instance *federatoraiv1alpha1.AlamedaService,
	gcIns *rbacv1.ClusterRole, asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.ValidatingWebhookConfigurationList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating ValidatingWebhookConfiguration...", "name",
			...
			log.Info("Updating ValidatingWebhookConfiguration", "name",
				validatingWebhookConfiguration.Name)
			err = r.client.Update(context.TODO(), &copyInstance)
			if err != nil {
			...
			log.Info("Updating ValidatingWebhookConfiguration done.", "name",
				validatingWebhookConfiguration.Name)
		}
	}
	return nil
...
func (r *ReconcileAlamedaService) syncDeployment(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.DeploymentList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource Deployment... ",
			...
			log.Info("Successfully Creating Resource Deployment",
				"resourceDep.Name", resourceDep.Name)
			continue
		} else if err != nil {
			...
			if updateresource.MisMatchResourceDeployment(
				...
				log.Info("Update Resource Deployment:",
				...
				log.Info("Successfully Update Resource Deployment",
					"resourceDep.Name", foundDep.Name)
			}
		}
	}
...
func (r *ReconcileAlamedaService) syncRoute(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.RouteList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource Route... ",
			...
			log.Info("Successfully Creating Resource route",
				"resourceRT.Name", resourceRT.Name)
		} else if err != nil {
...
func (r *ReconcileAlamedaService) syncIngress(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.IngressList {
		...
		if err := controllerutil.SetControllerReference(
			instance, resourceIG, r.scheme); err != nil {
			return errors.Errorf(
				"Fail resourceIG SetControllerReference: %s", err.Error())
		}
		foundIG := &extensionsv1beta1.Ingress{}
		err := r.client.Get(context.TODO(), types.NamespacedName{
			Name: resourceIG.Name, Namespace: resourceIG.Namespace,
		}, foundIG)
		if err != nil && k8sErrors.IsNotFound(err) {
			log.Info("Creating a new Resource Route... ",
				"resourceIG.Name", resourceIG.Name)
			err = r.client.Create(context.TODO(), resourceIG)
			if err != nil {
				return errors.Errorf("create route %s/%s failed: %s",
					resourceIG.Namespace, resourceIG.Name, err.Error())
			}
			log.Info("Successfully Creating Resource route",
				"resourceRT.Name", resourceIG.Name)
		} else if err != nil {
			return errors.Errorf("get route %s/%s failed: %s",
				resourceIG.Namespace, resourceIG.Name, err.Error())
		}
...
func (r *ReconcileAlamedaService) syncStatefulSet(
	instance *federatoraiv1alpha1.AlamedaService,
	asp *alamedaserviceparamter.AlamedaServiceParamter,
	resource *alamedaserviceparamter.Resource) error {
	for _, FileStr := range resource.StatefulSetList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			...
			log.Info("Update Resource StatefulSet:",
				"resourceSS.Name", resourceSS.Name)
			if updateresource.MisMatchResourceStatefulSet(foundSS, resourceSS) {
				...
				log.Info("Successfully Update Resource StatefulSet",
					"name", foundSS.Name)
			}
...
func (r *ReconcileAlamedaService) uninstallStatefulSet(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallIngress(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.IngressList {
		...
		if err != nil && k8sErrors.IsNotFound(err) {
			...
			return errors.Errorf("delete ingress %s/%s failed: %s",
				resourceIG.Namespace, resourceIG.Name, err.Error())
		}
	}
	return nil
...
func (r *ReconcileAlamedaService) uninstallRoute(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallDeployment(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.DeploymentList {
		resourceDep := componentConfig.NewDeployment(fileString)
...
func (r *ReconcileAlamedaService) uninstallService(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.ServiceList {
...
func (r *ReconcileAlamedaService) uninstallConfigMap(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.ConfigMapList {
		resourceCM := componentConfig.NewConfigMap(fileString)
...
func (r *ReconcileAlamedaService) uninstallSecret(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallServiceAccount(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallClusterRole(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallClusterRoleBinding(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallAlamedaScaler(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.AlamedaScalerList {
		resourceScaler := componentConfig.NewAlamedaScaler(fileString)
...
func (r *ReconcileAlamedaService) uninstallMutatingWebhookConfiguration(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.MutatingWebhookConfigurationList {
		mutatingWebhookConfiguration, err :=
...
func (r *ReconcileAlamedaService) uninstallValidatingWebhookConfiguration(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
	for _, fileString := range resource.AlamedaScalerList {
		validatingWebhookConfiguration, err :=
...
func (r *ReconcileAlamedaService) uninstallScalerforAlameda(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallPersistentVolumeClaim(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallDaemonSet(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallPodSecurityPolicy(
	instance *federatoraiv1alpha1.AlamedaService,
	resource *alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) uninstallResource(
	resource alamedaserviceparamter.Resource) error {
...
func (r *ReconcileAlamedaService) isNeedToBeReconciled(
	alamedaService *federatoraiv1alpha1.AlamedaService,
...
func (r *ReconcileAlamedaService) updateAlamedaServiceActivation(
...
func (r *ReconcileAlamedaService) updateAlamedaService(
	alamedaService *federatoraiv1alpha1.AlamedaService,
...
func (r *ReconcileAlamedaService) updateAlamedaServiceStatus(
	...
	log.Info("Update AlamedaService Status Successfully",
		"resource.Name", copyAlamedaService.Name)
	return nil
}

func (r *ReconcileAlamedaService) updateAlamedaServiceAnnotations(
	alamedaService *federatoraiv1alpha1.AlamedaService,
	...
	log.Info("Update AlamedaService Annotations Successfully",
		"resource.Name", copyAlamedaService.Name)
	return nil
}

func (r *ReconcileAlamedaService) checkAlamedaServiceSpecIsChange(
	alamedaService *federatoraiv1alpha1.AlamedaService,
...
func (r *ReconcileAlamedaService) isAlamedaServiceFirstReconciledDone(
...
func (r *ReconcileAlamedaService) deleteDeploymentWhenModifyConfigMapOrService(
...
func (r *ReconcileAlamedaService) patchConfigMapResourceVersionIntoPodTemplateSpecLabel(
...
func (r *ReconcileAlamedaService) removeUnsupportedResource(
