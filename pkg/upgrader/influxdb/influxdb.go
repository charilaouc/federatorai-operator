...
func InfluxdbUpgrade(timeout string, k8sClient client.Client) error {
	err := parseFlag(timeout)
	if err != nil {
		return errors.Wrap(err, "parse flag failed")
	}
	return removeInfluxDeploy(k8sClient)
}

func parseFlag(timeout string) error {
	var err error
...
func removeInfluxDeploy(k8sClient client.Client) error {
	ctx, cancel := context.WithTimeout(context.Background(), upgradeTimeout)
	defer cancel()
	alamedaServices, err := listAlamedaServices(ctx, k8sClient)
	if err != nil {
	...
	for _, alamedaService := range alamedaServices {
		...
		exist, err := isAlamedaInfluxdbServiceExist(ctx, k8sClient, service)
		if err != nil {
...
func upgradeFromKiloToLima(k8sClient client.Client) error {
...
func listAlamedaServices(ctx context.Context, k8sClient client.Client) (
	[]federatoraiv1alpha1.AlamedaService, error) {
...
func isAlamedaInfluxdbServiceExist(ctx context.Context, k8sClient client.Client,
	service corev1.Service) (bool, error) {
...
func scaleAndWaitWorkloadControllers(ctx context.Context, controllers []workloadController, replicas int32) error {
	...
	for _, controller := range controllers {
		...
		wg.Go(func() error {
			...
			switch string(copyController.Kind) {
